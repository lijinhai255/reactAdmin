...
class Login extends React.Component {
    ...
    componentDidUpdate(prevProps) { // React 16.3+弃用componentWillReceiveProps
        console.log(this.props,"登陆页面监听this.props")
        const users = Storage.getItem("loginUser") && JSON.parse(Storage.getItem("loginUser"))

        const { state} = this.props
        if (state.status === 0 && state.message !== "" && users.status===0){
            let search = this.props.location.search
            setTimeout(()=>{
                !search.length === 2 ? this.props.history.push(decodeURI(search.slice(1))) : this.props.history.push("/app/datas/detail");
                },1000)
